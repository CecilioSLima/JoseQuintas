...
FUNCTION MenuCria( lInterno )
   ...
   MenuOption( "Integração" )
      ...
      MenuOption( "XML de NFE" )
         ...
         MenuOption( "Conversão em Excel",       "PEDIXLS" )
   ...
   MenuOption( "Sistema" )
      ...
      MenuOption( "Configurações/Atualizações" )
         ...
         MenuOption( "Contábil Diário Livro/Folha",   "PCONTNUMDIA" )
...
/*
FUNCTION PTesZeraEsto()

   LOCAL nSaldo, mNumLan

   IF ! MsgYesNo( "Confirma zerar estoque 31/03/2019" )
      RETURN NIL
   ENDIF
   ...
   DO WHILE ! Eof()
      ...
      SEEK jpitem->ieItem
      nSaldo := 0
      DO WHILE jpitem->ieItem == jpestoq->esItem .AND. jpestoq->esDatLan <= Stod( "20190331" ) .AND. ! Eof()
         nSaldo += iif( jpestoq->esTipLan == "1", -1, 1 ) * jpestoq->esQtde
         SKIP
      ENDDO
      IF Val( Str( nSaldo ) ) != 0
         OrdSetFocus( "numlan" )
         mNumLan := NovoCodigo( "jpestoq->esNumLan" )
         RecAppend()
         REPLACE ;
            jpestoq->esNumLan WITH mNumLan, ;
            jpestoq->esDatLan WITH Stod( "20190331" ), ;
            jpestoq->esTipLan WITH Iif( nSaldo < 0, "2", "1" ), ;
            jpestoq->esNumDep WITH "1", ;
            jpestoq->esNumDoc WITH "BLOCOK", ;
            jpestoq->esItem   WITH jpitem->ieItem, ;
            jpestoq->esTransa WITH "", ;
            jpestoq->esCliFor WITH "", ;
            jpestoq->esQtde   WITH Abs( nSaldo ), ;
            jpestoq->esValor  WITH 0, ;
            jpestoq->esCfOp   WITH iif( nSaldo < 0, "1.102", "5.102" ), ;
            jpestoq->esTipDoc WITH "BLOCOK", ;
            jpestoq->esPedido WITH "BLOCOK", ;
